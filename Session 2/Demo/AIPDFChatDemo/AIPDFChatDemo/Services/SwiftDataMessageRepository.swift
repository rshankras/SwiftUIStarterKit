...
class SwiftDataMessageRepository: MessageRepositoryProtocol {
    ...
    func saveMessage(_ message: Message) {
        print("SwiftDataMessageRepository: Saving message with ID \(message.id)")
        print("  - Content: \(message.content.prefix(50))...")
        print("  - Sender: \(message.sender)")
        print("  - PDF: \(message.pdfFileName ?? "none")")
        
        modelContext.insert(message)
        
        do {
            try modelContext.save()
            print("SwiftDataMessageRepository: Successfully saved message")
        } catch {
            print("SwiftDataMessageRepository: Error saving message: \(error.localizedDescription)")
        }
    }
